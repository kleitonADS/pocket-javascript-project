...
export async function getWeekPendingGoals(){
    ...
    const pendingGoals = await db.with(goalCreateUpToWeek, goalsCompletionCounts).select({
        ...
        `.mapWith(Number),
    }).from(goalCreateUpToWeek).leftJoin(goalsCompletionCounts, eq(goalsCompletionCounts.goalId, goalCreateUpToWeek.id))
